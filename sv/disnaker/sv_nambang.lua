...
AddEventHandler('RW:axeBroke', function()
    if HasItem(source, 'pickaxe') >= 2 then
...
RegisterServerEvent('RW:phone')
AddEventHandler('RW:phone', function()
    if HasItem(source, 'phone') then
        RemoveItem(source, 'phone', 1)
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'You phone is water damaged, try dry off'})
    else
        Wait(2000)
    end
end)