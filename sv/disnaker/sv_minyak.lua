...
RegisterServerEvent('rw:Process2minyak')
AddEventHandler('rw:Process2minyak', function()
    local xPlayer = ESX.GetPlayerFromId(source)
	local ox_inventory = exports.ox_inventory
	...
	if luck >= 2 and luck <= 3 then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Kamu Tidak Memiliki Bahan'})
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Inven Kamu penuh mungkin'})
    end  
end)
